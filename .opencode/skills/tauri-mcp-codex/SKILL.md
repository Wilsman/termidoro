...
description: Configure and use the Tauri MCP server with OpenCode to control a Tauri app (build/run MCP server, wire the Tauri plugin/socket, set MCP env, and use available tools like screenshots, DOM, JS exec, input, and window/localStorage management).
metadata:
  short-description: Tauri MCP server + OpenCode usage
